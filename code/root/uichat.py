...
class ChatModeButton(ui.Window):
	...
	if app.ENABLE_WEREDNNBS_UIGAME:
		OUTLINE_COLOR = grp.GenerateColor(0.95294117, 0.83529411, 0.63921568, 1.0)
		OVER_COLOR = grp.GenerateColor(0.95294117, 0.83529411, 0.63921568, 0.3)
	else:
...
#				return
		...
		if text == "(DEscapeZirhliAt)":
			return

		if text == "(DEscapeAskerAt)":
			return
	...
	def __SendShoutChatPacket(self, text):
		...
		if net.GetEmpireID() == 1:
			self.__SendChatPacket("|cFFFF0000|H|h[Shinsoo]|cFFA7FFD4|H|h "+constInfo.pm_button+'|cFF47DA00|Hpm_button:'+str(player.GetName())+'|h[PM AT]|h|r'+" [Lv."+str(player.GetStatus(player.LEVEL))+"] ["+myChannel+"] : "+text[1:], chat.CHAT_TYPE_SHOUT)
		elif net.GetEmpireID() == 2:
			self.__SendChatPacket("|cFFFFFF00|H|h[Chunjo]|cFFA7FFD4|H|h "+constInfo.pm_button+'|cFF47DA00|Hpm_button:'+str(player.GetName())+'|h[PM AT]|h|r'+" [Lv."+str(player.GetStatus(player.LEVEL))+"] ["+myChannel+"] : "+text[1:], chat.CHAT_TYPE_SHOUT)
		elif net.GetEmpireID() == 3:
			self.__SendChatPacket("|cFF0080FF|H|h[Jinno]|cFFA7FFD4|H|h "+constInfo.pm_button+'|cFF47DA00|Hpm_button:'+str(player.GetName())+'|h[PM AT]|h|r'+" [Lv."+str(player.GetStatus(player.LEVEL))+"] ["+myChannel+"] : "+text[1:], chat.CHAT_TYPE_SHOUT)
...
class ChatInputSet(ui.Window):
	...
	if app.ENABLE_WEREDNNBS_UIGAME:
		CHAT_OUTLINE_COLOR = grp.GenerateColor(0.95294117, 0.83529411, 0.63921568, 1.0)
	else:
...
if app.ENABLE_WEREDNNBS_UIGAME:
	...
	class ChatWindow(ui.Window):
		...
		BOARD_END_COLOR = grp.GenerateColor(0.02352941, 0.01568627, 0.00392156, 0.8)
		BOARD_MIDDLE_COLOR = grp.GenerateColor(0.02352941, 0.01568627, 0.00392156, 0.5)
		CHAT_OUTLINE_COLOR = grp.GenerateColor(0.95294117, 0.83529411, 0.63921568, 1.0)
		...
		def __init__(self):
			...
			imgChatBarLeft.LoadImage("d:/ymir work/ui/gui/taskbar/chat_bar.tga")
			imgChatBarLeft.Show()
			...
			imgChatBarRight.LoadImage("d:/ymir work/ui/gui/taskbar/chat_bar.tga")
			...
			imgChatBarMiddle.LoadImage("d:/ymir work/ui/gui/taskbar/chat_bar.tga")
			imgChatBarMiddle.Show()
			self.imgChatBarMiddle = imgChatBarMiddle
			imgChatBarCenter = ui.ImageBox()
			imgChatBarCenter.SetParent(self.btnChatSizing)
			imgChatBarCenter.AddFlag("not_pick")
			imgChatBarCenter.LoadImage("d:/ymir work/ui/gui/taskbar/chat_bar_center.tga")
			imgChatBarCenter.Show()
			self.imgChatBarCenter = imgChatBarCenter
		...
		def __RefreshSizingBar(self):
			...
			self.imgChatBarCenter.SetPosition(width/2 - 37, 0)
