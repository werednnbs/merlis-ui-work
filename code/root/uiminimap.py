...
class AtlasWindow(ui.ScriptWindow):
	...
	def LoadWindow(self):
		...
		if app.ENABLE_WEREDNNBS_UIGAME:
			self.AtlasMainWindow.SetPosition(4, 40)
		else:
	...
	def Show(self):
		if self.AtlasMainWindow:
			...
			if bGet:
				if app.ENABLE_WEREDNNBS_UIGAME:
					self.SetSize(iSizeX + 0, iSizeY + 62)
				else:
				...
				if app.ENABLE_WEREDNNBS_UIGAME:
					self.board.SetSize(iSizeX + 9, iSizeY + 44)
				else:
...
class MiniMap(ui.ScriptWindow):
	...
	def enyakinisinlayicim(self):
		...
		questionDialog.SetText2(constInfo.yesilrenk+localeInfo.NEAREST_RINSER)
		questionDialog.Open()
