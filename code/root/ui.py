...
if app.ENABLE_WEREDNNBS_UIGAME:
	class TitleBar(Window):
		...
		def MakeTitleBar(self, width, color):
		    ...
		    imgLeft.LoadImage("d:/ymir work/ui/gui/title_bar/titlebar_left.tga")
		    imgCenter.LoadImage("d:/ymir work/ui/gui/title_bar/titlebar_center.tga")
		    imgRight.LoadImage("d:/ymir work/ui/gui/title_bar/titlebar_right.tga")
		    imgLeft.Show()
		    imgRight.Show()
		    imgCenter.Show()
		    ...
		    btnClose.SetUpVisual("d:/ymir work/ui/gui/title_bar/cBtn_normal.tga")
		    btnClose.SetOverVisual("d:/ymir work/ui/gui/title_bar/cBtn_hover.tga")
		    btnClose.SetDownVisual("d:/ymir work/ui/gui/title_bar/cBtn_press.tga")
		...
		def SetWidth(self, width):
		    self.imgCenter.SetRenderingRect(0.0, 0.0, float((width - self.BLOCK_WIDTH*2 + (self.PLUS_OVERFLOW*2) + (self.PLUS_POSITION*2)) - self.BLOCK_WIDTH) / self.BLOCK_WIDTH+0.5, 0.0)
		    self.imgCenter.SetPosition(self.BLOCK_WIDTH - self.PLUS_OVERFLOW - self.PLUS_POSITION-self.FIX, -self.FIX)
		    self.imgRight.SetPosition((width - self.BLOCK_WIDTH) + self.PLUS_POSITION, -self.FIX)
		    self.imgLeft.SetPosition(self.PLUS_POSITION*-1, -self.FIX)
...
if app.ENABLE_WEREDNNBS_UIGAME:
	class HorizontalBar(Window):
		...
		def SetWidth(self, width):
			self.imgCenter.SetRenderingRect(0.0, 0.0, float((width - self.BLOCK_WIDTH*0) - self.BLOCK_WIDTH) / self.BLOCK_WIDTH, 0.0)
...
if app.ENABLE_WEREDNNBS_UIGAME:
	class Board(Window):
		...
		def __init__(self):
			...
			self.MakeBoard("d:/ymir work/ui/gui/board/Board_Corner_", "d:/ymir work/ui/gui/board/Board_Line_")
			self.MakeBase()

		def MakeBoard(self, cornerPath, linePath):

			CornerFileNames = [ cornerPath+dir+".tga" for dir in ("lt", "lb", "rt", "rb", ) ]
			LineFileNames = [ linePath+dir+".tga" for dir in ("l", "r", "t", "b", ) ]
		...
		def MakeBase(self):
			...
			self.Base.LoadImage("d:/ymir work/ui/gui/Board/Board_Base.tga")
			self.Base.SetParent(self)
			self.Base.SetPosition(self.CORNER_WIDTH-self.FIX, self.CORNER_HEIGHT-self.FIX)
			self.Base.Show()

			self.decoration_bottom = ExpandedImageBox()
			self.decoration_bottom.AddFlag("not_pick")
			self.decoration_bottom.LoadImage("d:/ymir work/ui/gui/Board/detail_bottom.tga")
			self.decoration_bottom.SetParent(self)
			self.decoration_bottom.Show()
		...
		def SetSize(self, width, height):
			...
			self.Corners[self.LB].SetPosition(-self.FIX, height - self.CORNER_HEIGHT+self.FIX)
			self.Corners[self.LT].SetPosition(-self.FIX, -self.FIX/2)
			self.Corners[self.RT].SetPosition(width - self.CORNER_WIDTH+self.FIX, -self.FIX/2)
			self.Corners[self.RB].SetPosition(width - self.CORNER_WIDTH+self.FIX, height - self.CORNER_HEIGHT+self.FIX)
			self.Lines[self.R].SetPosition(width - self.CORNER_WIDTH+self.FIX, self.CORNER_HEIGHT-self.FIX)
			self.Lines[self.L].SetPosition(-self.FIX, self.CORNER_HEIGHT-self.FIX)
			self.Lines[self.T].SetPosition(self.CORNER_WIDTH-self.FIX, -self.FIX/2)
			self.Lines[self.B].SetPosition(self.CORNER_HEIGHT-self.FIX, height - self.CORNER_HEIGHT+self.FIX)

			verticalShowingPercentage = float((height - self.CORNER_HEIGHT*2+self.FIX*2) - self.LINE_HEIGHT) / self.LINE_HEIGHT
			horizontalShowingPercentage = float((width - self.CORNER_WIDTH*2+self.FIX*2) - self.LINE_WIDTH) / self.LINE_WIDTH
			...
			self.decoration_bottom.SetPosition(width/2-32, height-42+self.FIX)
else:
	class Board(Window):
...
if app.ENABLE_WEREDNNBS_UIGAME:
	class BoardWithTitleBar(Board):
		def __init__(self):
			...
			titleBar.SetPosition(7, 7)
			titleBar.Show()

			titleName = TextLine()
			titleName.SetParent(titleBar)
			titleName.SetPosition(7, 2)
			titleName.SetWindowHorizontalAlignLeft()
			titleName.SetHorizontalAlignLeft()
			titleName.Show()
...
if app.ENABLE_WEREDNNBS_UIGAME:
	class ThinBoard(Window):
		...
		LINE_WIDTH = 25
		LINE_HEIGHT = 25
		BOARD_COLOR = grp.GenerateColor(0.01568627, 0.01176470, 0.00392156, 0.50)
		...
		def __init__(self, layer = "UI"):
			...
			CornerFileNames = [ "d:/ymir work/ui/gui/thinboard/corner_"+dir+".tga" for dir in ["LeftTop","LeftBottom","RightTop","RightBottom"] ]
			LineFileNames = [ "d:/ymir work/ui/gui/thinboard/line_"+dir+".tga" for dir in ["left","right","top","bottom"] ]
...
if app.ENABLE_WEREDNNBS_UIGAME:
	class NewGauge(Window):
		...
		def MakeGauge(self, width, color):
			...
			imgSlotLeft = ImageBox()
			imgSlotLeft.SetParent(self)
			imgSlotLeft.LoadImage("%s/gauge/gauge_slot_left.tga" % PATCH_COMMON)
			imgSlotLeft.Show()

			imgSlotRight = ImageBox()
			imgSlotRight.SetParent(self)
			imgSlotRight.LoadImage("%s/gauge/gauge_slot_right.tga" % PATCH_COMMON)
			imgSlotRight.Show()
			imgSlotRight.SetPosition(width - self.SLOT_WIDTH, 0)

			imgSlotCenter = ExpandedImageBox()
			imgSlotCenter.SetParent(self)
			imgSlotCenter.LoadImage("%s/gauge/gauge_slot_center.tga" % PATCH_COMMON)
			imgSlotCenter.Show()
			imgSlotCenter.SetRenderingRect(0.0, 0.0, float((width - self.SLOT_WIDTH*2) - self.SLOT_WIDTH) / self.SLOT_WIDTH, 0.0)
			imgSlotCenter.SetPosition(self.SLOT_WIDTH, 0)
			...
			imgGauge = ExpandedImageBox()
			imgGauge.SetParent(self)
			imgGauge.LoadImage("d:/ymir work/ui/gui/common/gauge/gauge_" + color + ".tga")
			imgGauge.Show()
			imgGauge.SetRenderingRect(0.0, 0.0, 0.0, 0.0)
			imgGauge.SetPosition(self.GAUGE_TEMPORARY_PLACE, 0)
	...
	class SpecialEditLine(EditLine):
		...
		def SetPlaceHolderText(self, text):
			EditLine.OnMouseLeftButtonDown(self)
	...
	class Input(Window):
		...
		def __init__(self, layer = "UI"):
			...
			self.__dictImages['LEFT'].LoadImage("%s/input/board_left_01_normal.tga" % PATCH_COMMON)
			self.__dictImages['CENTER'].LoadImage("%s/input/board_center_01_normal.tga" % PATCH_COMMON)
			self.__dictImages['RIGHT'].LoadImage("%s/input/board_right_01_normal.tga" % PATCH_COMMON)
	...
	class TextLink(Window):
	    ...
	    ## COLORS
		NORMAL_COLOR =  dates.COLOR_NORMAL
		OVER_COLOR = dates.COLOR_HOVER
		DOWN_COLOR = dates.COLOR_NORMAL
